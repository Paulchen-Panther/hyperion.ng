...
if(USE_COMPILER_CACHE)
    message(STATUS "Using compiler cache         : ${LAUNCHER} (${CACHE_PROG})")
endif()
...
if(OPENSSL_FOUND AND OPENSSL_VERSION)
    ...
    message(STATUS "OpenSSL version              : OpenSSL NOT found (https webserver will not work)")
endif()
...
if(ENABLE_MDNS AND TARGET qmdns)
    get_target_property(QMDNS_VERSION qmdns INTERFACE_QMDNSENGINE_VERSION_PROPERTY)
    if(QMDNS_VERSION)
        message(STATUS "QMdnsEngine version          : ${QMDNS_VERSION}")
    endif()
endif()
if(ENABLE_PROTOBUF_SERVER AND TARGET ProtoBuf)
    get_target_property(PROTOBUF_VERSION ProtoBuf INTERFACE_PROTOBUF_VERSION_PROPERTY)
    if(PROTOBUF_VERSION)
...
if(ENABLE_FLATBUF_SERVER OR ENABLE_FLATBUF_CONNECT AND (TARGET FlatBuffers))
    get_target_property(FLATBUFFERS_VERSION FlatBuffers INTERFACE_FLATBUFFERS_VERSION_PROPERTY)
    if(FLATBUFFERS_VERSION)
        message(STATUS "FlatBuffers version          : ${FLATBUFFERS_VERSION}")
    endif()
endif()
if(ENABLE_DEV_NETWORK AND TARGET MbedTLS)
    get_target_property(MBEDTLS_VERSION MbedTLS INTERFACE_MBEDTLS_VERSION_PROPERTY)
    if(MBEDTLS_VERSION)
...
if(ENABLE_DEV_USB_HID)
    if(TARGET usb-1.0)
        get_target_property(LIBUSB_VERSION usb-1.0 LIBUSB_VERSION_PROPERTY)
        if(LIBUSB_VERSION)
...
if(ENABLE_DEV_FTDI)
	...
	pkg_check_modules(LIB_FTDI QUIET IMPORTED_TARGET libftdi1)
    if (LIB_FTDI_FOUND AND DEFINED LIB_FTDI_VERSION AND NOT LIB_FTDI_VERSION STREQUAL "")
        message(STATUS "LibFTDI version              : ${LIB_FTDI_VERSION}")
    endif()
endif()
